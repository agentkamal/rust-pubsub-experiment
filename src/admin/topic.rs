...
use crate::models::{
    publisher_client::PublisherClient, DeleteTopicRequest, GetTopicRequest, ListTopicsRequest, Topic,
};
...
impl TopicAdminClient {
    ...
    pub async fn get_topic(&self, topic: impl Into<String>) -> Result<Topic> {
        let mut client = self.client.clone();
        let request = GetTopicRequest { topic: topic.into() };
    ...
    pub async fn delete_topic(&self, topic: impl Into<String>) -> Result<()> {
        let mut client = self.client.clone();
        let request = DeleteTopicRequest { topic: topic.into() };
