...
use rust_pubsub::models::{
    publisher_client::PublisherClient, subscriber_client::SubscriberClient, DeleteTopicRequest,
    DetachSubscriptionRequest, DetachSubscriptionResponse, GetTopicRequest,
    ...
    PublishResponse, PubsubMessage, StreamingPullRequest, StreamingPullResponse, Topic,
    UpdateTopicRequest,
};
    ...
    async fn streaming_pull(
        ...
        tokio::spawn(async move {
            ...
            while sent < 1_000_000 {
                let msgs = (0..1000)
                    .map(|i| rust_pubsub::models::ReceivedMessage {
...
async fn start_mock_server() -> (
    ...
    let mock_publisher = MockPublisher {
        message_count: msg_count_pub.clone(),
    };
...
async fn benchmark_publisher_1m_msgs_10k_clients() {
    ...
    let pool = rust_pubsub::connection::ConnectionPool::new(16, Some(&addr))
        .await
        .unwrap();
    ...
    println!(
        "Publisher benchmark: {} msgs in {:?}, throughput: {} msgs/sec",
        total_msgs,
        elapsed,
...
async fn benchmark_subscriber_1m_msgs() {
    ...
    let elapsed = start.elapsed();
    println!(
        "Subscriber benchmark: {} msgs received in {:?}, throughput: {} msgs/sec",
        received,
        elapsed,
...
#[tokio::test(flavor = "multi_thread", worker_threads = 8)]
async fn benchmark_publisher_1gbps_limit() {
    let (_server, addr, msg_count, _, _) = start_mock_server().await;

    // Use a large connection pool to avoid saturation affecting the 1GB/s test
    let pool = rust_pubsub::connection::ConnectionPool::new(64, Some(&addr))
        .await
        .unwrap();

    let batching_settings = BatchingSettings {
        element_count_threshold: 1000,
        request_byte_threshold: 10_000_000, // 10MB
        delay_threshold: Duration::from_millis(10),
    };

    let publisher = Arc::new(Publisher::new(
        "test-topic-1gbps".into(),
        pool,
        batching_settings,
    ));
    ...
    // 1GB = 1000 MB. We'll send 100,000 messages of 10KB each = 1GB total data.
    let total_msgs = 100_000;
    let msgs_per_task = total_msgs / 100;
    let payload = vec![0u8; 10240]; // 10KB payload
    ...
    let elapsed = start.elapsed();
    assert_eq!(msg_count.load(Ordering::Relaxed), total_msgs);
    let total_bytes = total_msgs as u64 * 10240;
    let bytes_per_sec = total_bytes as f64 / elapsed.as_secs_f64();
    println!(
        "1GB/s test: {} msgs in {:?}, throughput: {:.2} GB/sec",
        total_msgs,
        elapsed,
...
#[tokio::test(flavor = "multi_thread", worker_threads = 8)]
async fn benchmark_connection_pool_saturation() {
    let (_server, addr, msg_count, _, _) = start_mock_server().await;

    // Saturated connection pool with only 1 connection
    ...
    let batching_settings = BatchingSettings {
        ...
        delay_threshold: Duration::from_millis(1),
    };

    let publisher = Arc::new(Publisher::new(
        "test-topic-saturation".into(),
        pool,
        batching_settings,
    ));
    ...
    let start = Instant::now();
    let mut handles = Vec::new();
    let total_msgs = 50_000;
    let msgs_per_task = total_msgs / 500;

    // Spawn 500 concurrent tasks trying to use the 1 connection
    for _ in 0..500 {
        let pub_clone = publisher.clone();
        handles.push(tokio::spawn(async move {
            for _ in 0..msgs_per_task {
    ...
    assert_eq!(msg_count.load(Ordering::Relaxed), total_msgs);
    println!(
        "Saturation test: {} msgs in {:?} with 1 connection",
        total_msgs, elapsed
    );
}
